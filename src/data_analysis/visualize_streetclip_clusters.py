# -*- coding: utf-8 -*-
"""
StreetCLIPエンベディング（平均済み）を読み込み、
階層的クラスタリングを実行し、結果を地図上に可視化するスクリプト。
"""
...
# --- パス設定 ---
# 集約済み（平均）エンベディングを使用
EMBEDDING_PATH = os.path.join(BASE_DIR, 'data', 'processed', 'streetclip_embeddings', 'streetclip_features_mean.csv')
...
def main():
    ...
    # --- 1. 階層的クラスタリング ---
    print("StreetCLIPエンベディングの階層的クラスタリングを実行中...")
    
    # 特徴量カラムを抽出 ('feat_' で始まる列)
    feature_cols = [c for c in df.columns if c.startswith('feat_')] 
    features = df[feature_cols].values
    
    # 標準化 (StreetCLIPの特徴量は既に正規化されていることが多いが、念のため)
    scaler = StandardScaler()
    features_scaled = scaler.fit_transform(features)

    # Ward法で階層的クラスタリングを実行
    # 次元数が大きい(768次元)ため、少し時間がかかる場合があります
    ...
    plt.title(f'Hierarchical Clustering Dendrogram of StreetCLIP Embeddings ({N_CLUSTERS} Clusters, Ward Linkage)')
    plt.xlabel('Point ID')
    plt.ylabel('Distance (Ward)')
    plt.tight_layout()
    dendrogram_output_path = os.path.join(OUTPUT_DIR, f'streetclip_dendrogram_{N_CLUSTERS}.png')
    ...
    map_output_path = os.path.join(OUTPUT_DIR, f'streetclip_cluster_map_{N_CLUSTERS}.html')
