...
class Phase3Recommender:
    def __init__(self):
        """フェーズ3専用推薦エンジンの初期化"""
        print("フェーズ3推薦エンジンを初期化中...")
    ...
    def recommend(self, target_poi_name, top_n=10):
        """
        起点POIから、「同POIクラスタ」かつ「同景観クラスタ」のPOIを推薦する
        """
        # 1. 基準となるPOIを検索
        ...
        candidates_df = self.poi_df[
            (self.poi_df['cluster'] == target_poi_cluster) & 
            (self.poi_df['ls_cluster'] == target_ls_cluster) &
            (self.poi_df['name'] != target_poi['name']) # 自身を除外
        ].copy()
        
        total_candidates = len(candidates_df)
        print(f"-> 条件（機能クラスタ: {target_poi_cluster}, 景観クラスタ: {target_ls_cluster}）に合致した候補POI総数: {total_candidates}件")
        
        if candidates_df.empty:
            print(f"警告: '{target_poi_name}' と同じ条件（POIクラスタ={target_poi_cluster}, 景観クラスタ={target_ls_cluster}）を満たす他のPOIが見つかりませんでした。")
        ...
        recommended_df = candidates_df.head(top_n)
        return recommended_df, target_poi, candidates_df

if __name__ == "__main__":
    recommender = Phase3Recommender()
    test_poi = "金森赤レンガ倉庫"
    res_df, t_poi = recommender.recommend(test_poi)
    if not res_df.empty:
        print("\n--- 推薦結果 ---")
