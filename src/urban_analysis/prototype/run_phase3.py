...
from src.urban_analysis.prototype.phase3_recommender import Phase3Recommender
...
def main(target_poi_name="函館朝市", start_time_str="10:00"):
    print("=== 函館観光ルート推薦 Phase 3 (ルールベースアプローチ) ===")
    ...
    try:
        # 3. 関連エリア・施設の特定（同POIクラスタ ＆ 同景観クラスタ）
        print("\n[2] 同機能クラスタかつ同景観クラスタのPOIを抽出中...")
        ...
        base_output_path = os.path.join(OUTPUT_DIR, f"phase3_route_map_{target_poi_name}.html")
        output_path = base_output_path
        counter = 1
        while os.path.exists(output_path):
            output_path = os.path.join(OUTPUT_DIR, f"phase3_route_map_{target_poi_name}_{counter}.html")
            counter += 1
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="観光ルート推薦 (Phase 3)")
