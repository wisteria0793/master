...
### アプローチ
1. **テキストベースのクラスタリング**:
   821件のPOI（閉店施設などのノイズを除外済みの797件）に対し、`Sentence-BERT`で生成した施設紹介文のエンベディング（768次元）を用いて階層的クラスタリング（Ward法）を行い、20個のテキストクラスタに分類。
2. **分布の可視化**:
...
3. **シルエットスコアによる定量評価**:
   ...
   - **意味的距離**: Sentence-BERTベクトル間のコサイン距離。
   - **物理的距離**: 緯度経度間のHaversine距離（球面距離）。

### 結果
- **シルエットスコアの推移**:
  クラスタ数（k）を2から40まで変化させ、意味的距離（コサイン）と物理的距離（Haversine）のシルエットスコアをそれぞれ算出した結果、以下の推移グラフが得られた。
  ![POI Text Silhouette Scores](./docs/results/poi_text_silhouette_scores.png)
- **最適なクラスタ数の選定**:
  意味的距離のスコア推移において、`k=12` 付近に明確な局所的ピーク（Score: `0.0453`）が確認された。`k=13` 以降でスコアが一度急落することから、施設紹介文は12個の主要な文脈（カテゴリ）に分類するのが自然言語的に最もまとまりが良いと判断し、以降の分析では **`k=12`** を採用した。
- **k=12 におけるスコア**:
  - **意味的距離のスコア**: `0.0453`
  - **物理的距離のスコア**: `-0.3415`

### 考察
意味的距離のスコアが0付近であることは、高次元の自然言語ベクトルにおいて、施設の意味がグラデーションのように連続的に分布していることを示しており妥当である。
最も重要な発見は、**物理的距離のスコアがマイナス（-0.3415）であること**である。これは、「同じような意味や機能を持つ施設（例: ジンギスカン、歴史的建造物、温泉など）が、特定のエリアに固まって存在するのではなく、函館市内にバラバラに分散している」ということを定量的に証明している。
この結果は、ルート推薦プロトタイプにおいて**「POIの選定（何を見るか）」は純粋なテキストの類似度で行い、「ルートの生成（どういう雰囲気を歩くか）」は景観クラスタで行う、という役割の分離アプローチが極めて正しいことの強力な裏付け**となる。
