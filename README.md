...
画像とテキストを同じベクトル空間で表現したいため、CLIPを使用。
  ...
  score: 0.3361, Dist: 21.5m
  ![長谷川ストア　ベイエリア店](./images/54265577347_0653c055f1.jpg)
   2 > Photo by [inunami](https://www.flickr.com/photos/94693136@N05/) / [CC BY 2.0](https://creativecommons.org/licenses/by/2.0/deed.en)<br>
...
3. 特になし
4. 2-2-3とは結果がかなり変わった。ただ、場所を見ると、函館市外のPOIばかりが上位に来ていることがわかる。やはり函館市の外になると画像数が劇的に減るため、画像がないことにより、テキストベクトルの影響値が大きいということがわかる。αが0.5と0.3で変わらなかったのがそれを表していると考える。<br>
4-2の結果では、α=0.5では4-1と結果は変わらなかったが、αを1に近づけると、2-2-3の結果に近づくという結果となった。<br>今回の手法（周辺画像の重み付け平均）は、「特定のPOIが持つユニークな視覚的特徴」を捉えるのではなく、「POIが位置するエリア全体の平均的な視覚的特徴」を抽出した。そして、その平均的特徴が、ルート推薦のような具体的な検索タスクにおいては、むしろノイズとして機能してしまった。
