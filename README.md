...
#### フェーズ3：観光ルート推薦エンジンの実装（優先開発事項）
ユーザー体験（UX）を軸とした、地区特性を活かしたルート推薦エンジンの構築。

- **推薦アルゴリズムの大まかな流れ (Main Flow)**:
    1. **[Step 1] 起点POIの選択と文脈理解**:
       ユーザーが選択した1地点から、その地点の「機能クラスター」と「周辺景観」をAIが認識し、地区の性格を特定。
    2. **[Step 2] 地区レベルでの提案 (Area Recommendation)**:
       同一地区内での深掘り、または隣接した「毛色の異なる地区（景観のギャップがある地区）」への転換を提示。
    3. **[Step 3] スポット候補の抽出 (POI Layering)**:
       選定された地区の中から、ユーザーの好みに似た、あるいは驚きを与える「乖離地点」などを中心に次の目的地候補を選定。
    4. **[Step 4] 景観最適化ルートの生成 (Landscape-aware Pathfinding)**:
       単なる最短経路ではなく、道路一本ごとの景観評価に基づき、**「最も歴史的な景観を長く楽しめる道」**や**「景観の変化が劇的な道」**を優先的に繋ぎ、ナビゲート。

- **将来的な高度化アプローチ**:
    - **埋め込みベクトル空間での連続遷移**: 地区の境界に縛られない、景観のグラデーションに基づいた推薦。
    - **ストーリー性・シーケンスの最適化**: 「物語的な景観の変化」や「意外性（Dissonance）の導入」を考慮した経路生成。
